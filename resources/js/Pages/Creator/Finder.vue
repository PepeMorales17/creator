...
import {
  ...
  SettingsIcon,
  ChevronsLeftIcon,
} from "@zhuowenli/vue-feather-icons/dist/vue-feather-icons.cjs";
  ...
  methods: {
    back() {
      if (this.selectedFolder) {
        ...
        const root =
          lastI > -1
            ? this.selectedFolder.root_index
                .substring(0, lastI)
                .replace(/\.children(?!.*\.children)/i, "")
            : this.selectedFolder.root_index;
    ...
    async getFilesFromFolder(f) {
      if (f.id === (!!this.selectedFolder ? this.selectedFolder.id : null))
        return;
        ...
        if (!!this.selectedFolder) {
          f =
            this.selectedFolder.children[
              this.selectedFolder.children.findIndex((x) => f.id == x.id)
            ];
        }
      }
    ...
    async deleted(file, index) {
      if (!!file.name) {
        await folder.delete(file).then((x) => {
          const root = this.selectedFolder
            ? this.selectedFolder.root_index
            : !!file.root_index
            ? file.root_index
            : null;
          if (root) {
            ...
            if (parentFolder) {
              if (parentFolder.children) {
                const ind = parentFolder.children.findIndex(
                  (x) => x.id == file.id
                );
  ...
  <AppLayout>
    <div class="grid grid-cols-12 gap-6 mt-8">
      <div class="col-span-12 lg:col-span-3 2xl:col-span-2">
        <h2
          class="intro-y text-lg font-medium mr-auto mt-2 flex justify-between"
        >
          Finder
          <div
            class="w-full sm:w-auto text-blue-900 cursor-pointer"
            v-if="!!selectedFolder"
            @click="back"
          >
            {{ "<-" }} Atras
      ...
      <div class="col-span-12 lg:col-span-9 2xl:col-span-10">
        ...
        <div class="intro-y flex flex-col-reverse sm:flex-row items-center">
          ...
          <div class="w-full sm:w-auto flex-grow p-4 font-bold">
            Folder: {{ !!selectedFolder ? selectedFolder.root : null }}
          </div>
          <div class="w-full sm:w-auto flex">
            <ModalUpload
              @freshFiles="updateFiles"
              ...
              <div class="dropdown-menu w-40">
                <ul class="dropdown-content">
                  <li>
                    ...
                    >
                      <template #toggle
                        ><FolderPlusIcon class="w-4 h-4 mr-2" /> Nueva
                        carpeta</template
                      >
                      <template #content="{ modal }">
                        <label for="crud-form-1" class="form-label"
                          >Nombre del folder</label
                        >
                        <input
                          id="crud-form-1"
        ...
        <div class="intro-y grid grid-cols-12 gap-3 sm:gap-6 mt-5">
          <template v-if="!hideFiles">
            <FileView
              :file="file"
              @deleted="deleted(file, index)"
              @folderSelected="getFilesFromFolder($event)"
              v-for="(file, index) in showingFiles"
              :key="index"
              :actions="true"
            ></FileView>
          </template>
